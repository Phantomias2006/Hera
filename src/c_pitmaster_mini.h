...
void set_pitmaster(bool init) {
  ...
  PitServo[0].attach(pitMaster[0].io[1], 3, 0, 360);

  // IOs Pitmaster1
  pinMode(PITMASTER1IO1, OUTPUT);
  digitalWrite(PITMASTER1IO1, LOW);
  pitMaster[1].io[0] = PITMASTER1IO1;
  pinMode(PITMASTER1IO2, OUTPUT);
  digitalWrite(PITMASTER1IO2, LOW);
  pitMaster[1].io[1] = PITMASTER1IO2;
  PitServo[1].attach(pitMaster[1].io[1], 4, 0, 360);

  // Initialize Pitmaster0
  init_pitmaster(init, 0);
  // Initialize Pitmaster1
...
void aktor_limits(byte id, byte pio) {
  ...
  switch (aktor) {
    ...
    case SERVO:   // SERVO
      ...
      pitMaster[id].dcmin = map(dcmin,0,1000,0,360);
      pitMaster[id].dcmax = map(dcmax,0,1000,0,360);
      break;   
  }
}
      ...
      for (byte pp = 0; pp < 2; pp++) {      // pro Pitmaster jeweils 2 IO
        ...
        switch (aktor) {
          ...
          case FAN:  
            ...
            if (value == 0) {   
              ...
              if (millis() - pitMaster[id].timer0 < pitMaster[id].pause*2 && value < 30) { 
                analogWrite(pitMaster[id].io[pp],map(30,0,100,pitMaster[id].dcmin,pitMaster[id].dcmax), pitMaster[id].dcmax);   // BOOST
              } else {
                analogWrite(pitMaster[id].io[pp],map(value,0,100,pitMaster[id].dcmin,pitMaster[id].dcmax), pitMaster[id].dcmax);
              }
            }
            break;
          ...
          case SERVO:   // Achtung bei V2 mit den 12V bei Anschluss an Stromversorgung
            ...
            PitServo[pitMaster[id].id].write(smsec);
            PMPRINTF("SVO: %u,\t %u,\t %d\r\n", id, pitMaster[id].io[1], smsec);
            break;   
        }
      } 
    }
